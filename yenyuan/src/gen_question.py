...
qhead_map = {"GPE": "Where", "PERSON": "Who", "ORGANIZATION": "What", "DATE": "When", "MONEY": "How much", "LOCATION": "Where"}
...
def clean_sentence(tree):
    ...
    tree = tsurgeon.relabel_NP_in_PP(tree)
    return tree
...
def gen_question_recur(tree, ori_sent_inversed, ori_sent, questions, all_entities, prep):
    ...
    for i in range(0, len(tree)):
        ...
        if isinstance(node, nltk.Tree):
            ...
            if node.label() == tsurgeon.label_NP_in_PP:  # NP in PP
                # where
